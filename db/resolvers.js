...
const resolvers = {
  Query: {
    ...
    getClient: async (_, { id }, ctx) => {
      ...
      if (client.salesMan.toString() !== ctx.user.id) throw new Error("Not Authorized")
      return client;
    },
    ...
    getOrder: async (_, { id }, ctx) => {
      ...
      if (order.salesMan.toString() !== ctx.user.id) throw new Error("Not Authorized")
      return order;
    },
  ...
  Mutation: {
    ...
    authentification: async (_, { input }) => {
      ...
      //Check the Pass
      const isPassWordCorrect = await bcryptjs.compare(password, targetUser.password)
    ...
    newOrder: async (_, { input }, ctx) => {
      ...
      if (existsingClient.salesMan.toString() !== ctx.user.id) throw new Error("Not Authorized")
